...
export function PackageGrid({ initialPackages }: PackageGridProps) {
    ...
    const filteredPackages = useMemo(() => {
        return initialPackages.filter((pkg) => {
            ...
            // 4. Type Filtering
            if (typeFilter !== 'All') {
                if ((pkg.type || '').toLowerCase() !== typeFilter.toLowerCase()) return false;
            }

            return true;
        });
    }, [initialPackages, searchTerm, priceFilter, durationFilter, typeFilter]);

    const hasActiveFilters = searchTerm !== '' || priceFilter !== 'All' || durationFilter !== 'All' || typeFilter !== 'All';
    ...
    const animationKey = `${searchTerm}-${priceFilter}-${durationFilter}-${typeFilter}`;
    ...
    return (
        <div className="w-full">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-4">
                ...
                {/* Enhanced Search Bar & Filter Toggle */}
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    ...
                    <button 
                        ...
                        className={`flex items-center justify-center gap-2 px-5 py-3 rounded-2xl font-semibold transition-all shadow-sm ${showFilters || hasActiveFilters ? 'bg-brand-primary text-brand-navy border-2 border-brand-primary shadow-brand-primary/20' : 'bg-white border-2 border-slate-100 text-slate-700 hover:border-brand-primary/50'}`}
                    >
                        <svg className="w-5 h-5 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
                        </svg>
                        Filters
                        {hasActiveFilters && <span className="flex items-center justify-center w-5 h-5 bg-brand-navy text-white text-[11px] rounded-full ml-1 font-bold">!</span>}
                    </button>
                </div>
            </div>
            ...
            {showFilters && (
            <div className="animate-fade-in flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8 p-5 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm">
                <div className="flex flex-col lg:flex-row gap-6 xl:gap-8 flex-1">
                    {/* Type Filter */}
                    <div className="flex items-center gap-3 flex-wrap">
                        <span className="text-sm font-semibold text-slate-700 uppercase tracking-wider">Type</span>
                        <div className="flex flex-wrap items-center gap-2">
                            {['All', 'Domestic', 'International'].map(opt => (
                                <button
                                    key={opt}
                ...
                <div className="flex items-center gap-4 border-t xl:border-t-0 pt-4 xl:pt-0 border-slate-200 mt-4 xl:mt-0 xl:pl-4">
                    {hasActiveFilters && (
                        <button 
                            onClick={clearAllFilters}
                            className="text-sm text-red-600 hover:text-red-700 font-semibold px-4 py-2 hover:bg-red-50 rounded-full transition-colors whitespace-nowrap"
                        >
                            Clear all filters
                        </button>
                    )}
